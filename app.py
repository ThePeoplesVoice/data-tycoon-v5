...
class Player:
    def __init__(self, name):
        ...
        self.orbital_swarms = 0
        self.efficiency = 1.0
        self.prestige_bonus = 0.0
    ...
    def link_real_starlink(self, hardware_id="DEMO-DISH-001"):
        ...
        self.currency += 750
        return {"latency_ms": 28, "laser_links": 42, "uptime": 99.8, "coverage": "Global"}

    def process_tick(self, demand, alliance_bonus=0.0):
        revenue = (self.data_centers * 10 + self.ai_clusters * 20 + self.edge_nodes * 5)
        if self.starlink_terminals > 0: revenue *= 1.4
        if self.orbital_swarms > 0: revenue *= 8.5
        revenue *= self.efficiency * (1 + self.prestige_bonus + alliance_bonus) * demand
        ep = revenue * 0.1
